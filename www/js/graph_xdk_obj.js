       ...
       //     app.consoleLog("campo="+campo+"modulo=",self.modulo);
                    switch (campo) {
                        ...
                        case 101: // max
                            if (self.modulo === null) {
                                valor = getObjects(valdata,
                                               "field"+self.series[0].campo+"_max");                                               } else
                            if (self.modulo >=0) {
                                var str=self.modulo+1;
                                var v_str=jsonPath(json_config,
                                            "$.node"+str+".field1_max");
  ...
  this.initChart = function() {
    ...
    self.data = new google.visualization.DataTable();
      if (self.tipo == 3) { // dounets
          ...
          for (var key in self.series) {
            app.consoleLog('key='+key+" titulo="+self.series[key].nome+" data="+self.series[key].campo);
            self.data.addColumn('number', self.series[key].nome);
            if (self.tipo==2 && flag==true) {// linha
                  self.data.addColumn({type: 'string', role: 'annotation'});
                  self.data.addColumn({type: 'string', role: 'annotationText', p: {html:true}});
                  flag=false;
                }
          }
          self.len_series=i;
    ...
    var yellow_value= range_val - (range_val * 0.25) + self.min_value;
      //console.log("range="+range_val+" min="+yellow_value+" red="+red_value+  "  max="+self.max_value +" min="+ self.min_value);
    var tick_value= 5;
    switch (self.tipo) {
        case 0: // gauge
    self.chart = new google.visualization.Gauge(document.getElementById(self.id_div));
    self.options = {width: self.largura, height: self.altura, min:self.min_value, max:self.max_value, redFrom: red_value, redTo: self.max_value, yellowFrom:yellow_value, yellowTo: red_value, minorTicks: tick_value};
            break;
        case 1: // area
   ...
   self.options = {
	...
	backgroundColor: 'white',
          hAxis: {title: 'tempo',  titleTextStyle: {color: 'blue'}, textStyle:{color:'black'}},
          vAxis: {title: 'watts',  titleTextStyle: {color: 'red'}, textStyle:{color:'black'}, minValue: 0}
        };
        break;
        case 2: // linha
            self.chart = new    google.visualization.LineChart(document.getElementById(self.id_div));
            self.options = { title: self.titulo, legend: { position: 'bottom' }, //interpolateNulls: true,
                            tooltip: {isHtml: true},
        ...
        case 3: // dounets
            ...
            self.options = { title: self.titulo, pieHole: 0.4 };
        break;
...
//        google.load('visualization', '1', {packages:['corechart']});
        break;
        case 3: // dounets
        this.nro_pontos =this.nro_pontos+1;
//        google.load('visualization', '1', {packages:['corechart']});
        break;
    }
 //   console.log("flag_getMainConfig="+flag_getMainConfig);
//    console.log("flag_entrou="+flag_entrou);
	//console.log("dev.isDeviceReady.f_browser_ready__="+dev.isDeviceReady.f_browser_ready__);
  //  if (true || flag_entrou)
        this.initChart();
//    else
